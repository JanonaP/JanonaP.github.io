...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    displayButton.addEventListener('click', () => {
        ...
        restaurants.forEach((restaurant, index) => {
            restaurantList.innerHTML += `<div>${restaurant.name}</div>`
        })
    ...
    findButton.addEventListener('click', (event) =>{
        ...
        if (bestRestaurant) {
            ...
            (${bestRestaurant.cuisine}) - Rating: ${bestRestaurant.rating}</div>`;
        } else {
            foundItem.innerHTML = '<div>Aw you couldn\'t find the best restaurant!</div>';
        }
        
        console.log('Nice you found the best restaurant! Now go eat.');
    });
});
